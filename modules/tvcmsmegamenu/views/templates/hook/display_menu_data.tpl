                ...
                {foreach from=$menus item=menu name=menus}
                    ...
                    {else}
                        {if isset($menu.type) && $menu.type == 'CAT' && $isit == 0}
                            ...
                            {$isit = 1}
                        {/if}
                        <li style="border-bottom: 1px solid #E5EDEF;{if isset($menu.type) && $menu.type == 'CAT'}padding: 5px 15px;{else}padding: 5px 30px;{/if}" class="level-1 {$menu.class|escape:'html':'UTF-8'}{if count($menu.sub_menu) > 0} parent{/if}" >
